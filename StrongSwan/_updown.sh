...
#
#
# Enter your subnet you want to have routed
SUBNETTOROUTE="192.168.1.0/24"
#
#
#
...
down-host:iptables)
	...
	;;
up-client:iptables)
  logger -t $TAG -p $FAC_PRIO "Bringing client up"
	...
	then
	  ...
	  logger -t $TAG -p $FAC_PRIO "Insert iptables rule -I FORWARD 1 -o $PLUTO_INTERFACE -p $PLUTO_PEER_PROTOCOL -s $PLUTO_MY_CLIENT $S_MY_PORT -d $PLUTO_PEER_CLIENT $D_PEER_PORT $IPSEC_POLICY_OUT -j ACCEPT"
    ...
    if ! [ -z $PLUTO_MY_SOURCEIP ]; then
      logger -t $TAG -p $FAC_PRIO "Insert iptables rule -I FORWARD 1 -i $PLUTO_INTERFACE -p $PLUTO_MY_PROTOCOL -s $PLUTO_PEER_CLIENT $S_PEER_PORT -d $PLUTO_MY_CLIENT $D_MY_PORT $IPSEC_POLICY_IN -j ACCEPT"
      iptables -t nat -I POSTROUTING 1 -s ${SUBNETTOROUTE} -d $PLUTO_PEER_CLIENT $D_MY_PORT -j SNAT --to $PLUTO_MY_SOURCEIP
    fi
    ...
    logger -t $TAG -p $FAC_PRIO "Insert iptables rule -I INPUT 1 -i $PLUTO_INTERFACE -p $PLUTO_MY_PROTOCOL -s $PLUTO_PEER_CLIENT $S_PEER_PORT -d $PLUTO_MY_CLIENT $D_MY_PORT $IPSEC_POLICY_IN -j ACCEPT"

	  iptables -I OUTPUT 1 -o $PLUTO_INTERFACE -p $PLUTO_PEER_PROTOCOL \
	      -s $PLUTO_MY_CLIENT $S_MY_PORT \
	      -d $PLUTO_PEER_CLIENT $D_PEER_PORT $IPSEC_POLICY_OUT -j ACCEPT

    logger -t $TAG -p $FAC_PRIO "Insert iptables rule -I OUTPUT 1 -o $PLUTO_INTERFACE -p $PLUTO_PEER_PROTOCOL -s $PLUTO_MY_CLIENT $S_MY_PORT -d $PLUTO_PEER_CLIENT $D_PEER_PORT $IPSEC_POLICY_OUT -j ACCEPT"
	fi
	#
	# allow IPIP traffic because of the implicit SA created by the kernel if
	# IPComp is used (for small inbound packets that are not compressed).
    ...
    logger -t $TAG -p $FAC_PRIO "Insert IPIP traffic rule -I INPUT 1 -i $PLUTO_INTERFACE -p 4 -s $PLUTO_PEER -d $PLUTO_ME $IPSEC_POLICY_IN -j ACCEPT"
	fi
	#
...
down-client:iptables)
  logger -t $TAG -p $FAC_PRIO "Bringing client down"
    ...
    logger -t $TAG -p $FAC_PRIO "Remove FORWARD iptables rule -D FORWARD -o $PLUTO_INTERFACE -p $PLUTO_PEER_PROTOCOL -s $PLUTO_MY_CLIENT $S_MY_PORT -d $PLUTO_PEER_CLIENT $D_PEER_PORT $IPSEC_POLICY_OUT -j ACCEPT"
    ...
    if ! [ -z $PLUTO_MY_SOURCEIP ]; then
      logger -t $TAG -p $FAC_PRIO "Remove FORWARD iptables rule -D FORWARD -i $PLUTO_INTERFACE -p $PLUTO_MY_PROTOCOL -s $PLUTO_PEER_CLIENT $S_PEER_PORT -d $PLUTO_MY_CLIENT $D_MY_PORT $IPSEC_POLICY_IN -j ACCEPT"

      iptables -t nat -D POSTROUTING -s ${SUBNETTOROUTE} -d $PLUTO_PEER_CLIENT $D_MY_PORT -j SNAT --to $PLUTO_MY_SOURCEIP
    fi
    ...
    logger -t $TAG -p $FAC_PRIO "Remove INPUT iptables rule -D INPUT -i $PLUTO_INTERFACE -p $PLUTO_MY_PROTOCOL -s $PLUTO_PEER_CLIENT $S_PEER_PORT -d $PLUTO_MY_CLIENT $D_MY_PORT $IPSEC_POLICY_IN -j ACCEPT"
	  ...
	  logger -t $TAG -p $FAC_PRIO "Remove INPUT iptables rule -D OUTPUT -o $PLUTO_INTERFACE -p $PLUTO_PEER_PROTOCOL -s $PLUTO_MY_CLIENT $S_MY_PORT -d $PLUTO_PEER_CLIENT $D_PEER_PORT $IPSEC_POLICY_OUT -j ACCEPT"
	fi
	#
	# IPIP exception teardown
	if [ -n "$PLUTO_IPCOMP" ]
	then
	  ...
	  logger -t $TAG -p $FAC_PRIO "Remove IPIP iptables rule -D INPUT -i $PLUTO_INTERFACE -p 4 -s $PLUTO_PEER -d $PLUTO_ME $IPSEC_POLICY_IN -j ACCEPT"
	fi
	#
